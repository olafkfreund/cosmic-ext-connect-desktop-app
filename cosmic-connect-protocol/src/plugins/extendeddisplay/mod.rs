...
use cosmic_display_stream::{
    EncoderConfig, InputHandler, StreamConfig, StreamingServer, TouchAction, TouchEvent,
    VideoEncoder, VideoTransform,
};
...
pub struct ExtendedDisplayPlugin {
    ...
    /// Video encoder (GStreamer pipeline)
...
impl ExtendedDisplayPlugin {
    ...
    pub async fn start_session(&mut self, device_id: &str, capabilities: &str) -> Result<()> {
        ...
        self.streaming_server = Some(server);
    ...
    pub async fn stop_session(&mut self, device_id: &str) -> Result<()> {
        ...
        if let Some(mut server) = self.streaming_server.take() {
            if let Err(e) = server.stop().await {
                warn!("Error stopping streaming server: {}", e);
            }
        }

        // Drop encoder and input handler
        self.encoder = None;
